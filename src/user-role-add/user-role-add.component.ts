...
@Component({
  ...
  styleUrls: [('./user-role-add.component.css').toString()]
})
...
export class UserRoleAddComponent implements OnInit{
  ...
  public hidePermissions: boolean;


  constructor(private cookieService: CookieService, private http:Http,private _localStorageService:LocalStorageService, private router: Router) {

  	this.hidePermissions = true;
  	this.currentLanguage = this._localStorageService.get("CurrentLanguage");

  	this.http.get(environment.apikey+"/generateJsonUrl.php?action=formRole&language="+ this.currentLanguage)
    .map(res => res.json())
    .subscribe(data => {
      if(!!data){
  ...
  checkAnyValueSelected(){
  	...
  	this.page_fields.forEach((value,key) => {
      value = value;
	    if(this.page_fields[key].selected == true){
	    	anySelected = 1;
	    }
	  });
  	...
  	this.operation_pages.forEach((value,key) => {
      value = value;
      if(this.operation_pages[key].selected == true){
      	anySelected = 1;
      }
  	});
  	...
  	this.pages.forEach((value,key) => {
	    if(this.pageRole['page_id'] == value.id){
	    	if(this.checkAnyValueSelected()){
		    	Object.assign(this.pages[key], {selected:true,selected_opr: this.operation_pages,selected_viewfields:this.page_fields});
		    	this.page_fields = [];
