...
declare var $: any;
@Component({
  selector: 'demandBookForm',
  templateUrl: './demandBookForm.component.html',
  styleUrls: [('./demandBookForm.component.css').toString()]

})
export class DemandBookFormComponent implements OnInit {
  ...
  DemandNumber: any;
  quantCheck: number;
  CSerialNoAvilable: any = [];
  ControlItemsArray: any = [];
  NCSerialNoAvilable: any = [];
  itemsArray: any = [];
  compareAvailSerialNo: any = [];
  Id: any;
  clicks: Number = 0;
  demandBookId: any;
  title: string;
  isReadOnly: Boolean = false;
  isReturnReadOnly: Boolean = false;
  ...
  removeDuplicateUsingFilter: Function;
  formName: string = 'Material Issue Form';
      ...
      if (this.statusList[statusValue].value == status) {
        switch (this.statusList[statusValue].value) {
          case '1':
            ...
            this.formName = 'Material Issue Form';
          ...
          case '3':
            ...
            this.formName = 'Under Repair Form';
            break;
          case '4':
            this.materialLoan = "";
            this.quantityLoan = "";
          ...
          case '5':
            ...
            this.formName = 'Loan Return Form';
            break;
        }
      }
    }
  ...
  updateLoanReturnMaterial(LoanReturnForm: any) {
    this.userPermissionInfo = JSON.parse(this._localStorageService.get('userPermission'));
    let userPermissionDataInfo = this.userPermissionInfo;
    let userRoleName = userPermissionDataInfo.RoleInfo.RoleName;
    var userCookie = JSON.parse(this.cookieService.get('user'));
  ...
  //   })
  // }
  //   getUnique(array: any, param:any){
  //     return array.filter(function(item:any, pos:any, array:any){
  ...
  addFieldsValue(material_id: any, quantity: any, formtype: any) {
    this.CSerialNoAvilable = [];
    this.NCSerialNoAvilable = [];
    this.ControlItemsArray = [];
    // this.issuedFieldsArray=[];
    // this.LoanFieldsArray=[];
    // this.issuedNonCtrlArray=[];
    // this.loanNonCtrlArray=[];
    let body1 = new URLSearchParams();
    body1.append('action', 'getCountItems');
    body1.append('quant', quantity);
    body1.append('material_id', material_id);
    this.http.post(environment.apikey + '/generateJsonUrl.php', body1)
      .map(res => res.json())
      .subscribe(data => {
        this.countData = data;
        this.quantCheck = this.countData.data;
        if (this.countData.code == '100') {
          if ((this.countData.hasOwnProperty('serialNo')) && (Array.isArray(this.countData.serialNo))) {
            for (let item in this.countData.serialNo) {
              this.CSerialNoAvilable.push(this.countData.serialNo[item]);
            }
            this.ControlItemsArray = this.CSerialNoAvilable;
            for (let i = 0; i < this.quantCheck; i++) {
              var object = {
                id: "",
                material_id: "",
                item_name: "",
              ...
              object.id = this.ControlItemsArray[i].id;
              object.material_id = this.ControlItemsArray[i].material_id;
              object.item_name = this.ControlItemsArray[i].item_name;
              object.item_part_no = this.ControlItemsArray[i].part_no;
              object.serial_no = this.ControlItemsArray[i].serial_no;
              if (formtype == 'Issue') {
          ...
          } else if ((this.countData.hasOwnProperty('serialNo')) && ((typeof (this.countData.serialNo) == 'object'))) {
            var objectNc = {
              id: "",
              material_id: "",
              item_name: "",
            ...
            objectNc.id = this.countData.serialNo.id;
            objectNc.material_id = this.countData.serialNo.material_id;
            objectNc.item_name = this.countData.serialNo.item_name;
            objectNc.item_part_no = this.countData.serialNo.part_no;
            objectNc.category = this.countData.serialNo.category;
          ...
          }     } else if (this.countData.code == '101') {
            alert("Items are Not Avilable");
          } else if (this.countData.code == '102') {
            if (+this.countData.data == 0) {
              alert("Items are Not Avilable");
            } else {
              alert("Only" + " " + this.countData.data + " " + "Items are Avilable");
            }
          }
      }, error => {
        console.log(error.json());
  ...
  deleteIssueFieldsValue(index: any, id: any, category: any) {
    if (category == 'Controllable') {
      this.issuedFieldsArray.splice(index, 1);
      let body = new URLSearchParams();
      body.append('action', 'deleteItem');
      body.append('id', id);
      this.http.post(environment.apikey + '/generateJsonUrl.php', body)
        .map(res => res.json())
        .subscribe(data => {
    ...
    } else if (category == 'Non-Controllable') {
  ...
  deleteLoanFieldsValue(index: any, id: any) {
  ...
  submitFaultyDemandNumber(DemandNumber: any) {
    ...
    this.http.post(environment.apikey + '/generateJsonUrl.php', body)
      ...
      .subscribe((res: any) => {
        if (!!res) {
          this.FaultyForm = res;
          for(let item in this.FaultyForm.store_item ){
            if(this.FaultyForm.store_item[item].quantity_nc == null){
            //  console.log(this.FaultyForm.store_item[item]);
              this.FaultyForm.store_item[item].id=this.FaultyForm.store_item[item].si_id;
              this.FaultyFieldsArray.push(this.FaultyForm.store_item[item]);
            }
          }
        //  console.log(this.FaultyFieldsArray);
        }
      }, error => {
        console.log(error.json());
      });
      ...
      .subscribe((res: any) => {
        if (!!res) {
          ...
          for(let item in this.RepairForm.store_item ){
            if(this.RepairForm.store_item[item].quantity_nc == null){
           //   console.log(this.RepairForm.store_item[item]);
              this.RepairForm.store_item[item].id=this.RepairForm.store_item[item].si_id;
              this.UnderRepairFieldsArray.push(this.RepairForm.store_item[item]);
            }
          }
          // this.UnderRepairFieldsArray = this.RepairForm.store_item;
     //     console.log(this.UnderRepairFieldsArray);
        }
      }, error => {
        console.log(error.json());
      });
  ...
  submitLoanReturnDemandNumber(DemandNumber: any) {
    ...
    this.http.post(environment.apikey + '/generateJsonUrl.php', body)
      ...
      .subscribe((res: any) => {
        if (!!res) {
          for(let item in res.store_item ){
            if(res.store_item[item].quantity_nc == null){
            //  console.log(res.store_item[item]);
              res.store_item[item].id=res.store_item[item].si_id;
              this.LoanReturnArray.push(res.store_item[item]);
            }
          }
        //  console.log(this.LoanReturnArray);
        }
      }, error => {
        console.log(error.json());
      });
        ...
        if (count == 0) {
          ...
          this.http.post(environment.apikey + '/generateJsonUrl.php', body)
            ...
            .subscribe(data => {
              if (data.code == 100) {
                this.demandBookId = data.materialDemandNo;
                this.router.navigate(['/print/issue', this.demandBookId]);
              } else {
                alert(data.message);
              }
            }, error => {
  ...
  addFaultyMaterial(FaultyMaterial: any) {
    ...
    this.http.post(environment.apikey + '/generateJsonUrl.php', body)
      ...
      .subscribe(data => {
        if (data.code == 100) {
          this.demandBookId = data.materialDemandNo;
          this.router.navigate(['/print/faulty', this.demandBookId]);
        } else {
          alert(data.message);
        }
      }, error => {
  ...
  addUnderRepairMaterial(UnderRepairMaterial: any) {
    ...
    this.http.post(environment.apikey + '/generateJsonUrl.php', body)
      ...
      .subscribe(data => {
        if (data.code == 100) {
          this.demandBookId = data.materialDemandNo;
          this.router.navigate(['/print/repair', this.demandBookId]);
        } else {
          alert(data.message);
        }
      }, error => {
  ...
  addLoanMaterial(LoanForm: any) {
    ...
    LoanForm.store_item = this.LoanFieldsArray;
    if (LoanForm.store_item.length == 0) {
          ...
          if (count == 0) {
            ...
            this.http.post(environment.apikey + '/generateJsonUrl.php', body)
              ...
              .subscribe(data => {
                if (data.code == 100) {
                  this.demandBookId = data.materialDemandNo;
                  this.router.navigate(['/print/LoanForm', this.demandBookId]);
                } else {
                  alert(data.message);
                }
              }, error => {
  ...
  addLoanReturnMaterial(LoanReturnForm: any) {
    ...
    this.http.post(environment.apikey + '/generateJsonUrl.php', body)
      ...
      .subscribe(data => {
        if (data.code == 100) {
          this.demandBookId = data.materialDemandNo;
          this.router.navigate(['/print/loanReturn', this.demandBookId]);
        } else {
          alert(data.message);
        }
      }, error => {
  ...
  closeAndRedirectToView() {
    this.router.navigate(['/demandPrint/view', this.demandBookId]);
  }
}
