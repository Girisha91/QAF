...
@Component({
  moduleId: module.id,
  selector: 'app-pabx-configurationInfo',
  templateUrl: './pabx-configurationInfo.component.html',
  styleUrls: [('./pabx-configurationInfo.component.css').toString()]
})
export class PabxConfigurationInfoComponent {
...
selectedDeactivateEmployee:any;
  ...
  exportToExcel(event:any) {
   ...
   this.excelService.download(dataArray,'export_Pabx_Sw');
  }
  ...
  public OnSubmit(formValue: any) {
    ...
    body.append('action', "configurationInfoList");
  ...
  loadData() {
    this.searchKeyArray = [{"item":"site_name","label":"Site Name"},
    {"item":"location","label":"Location"},
    {"item":"host_name","label":"Host Name"},
    {"item":"vlan_id","label":"Vlan Id"},
    {"item":"vlan_name","label":"Vlan Name"},
    {"item":"ip_address","label":"Ip Address"},
    {"item":"subnet_mask","label":"Subnet Mask"}];
...
fileList:any;
  ...
  import(){
    if (this.fileList.length > 0) {
      ...
      formData.append('module', "configuration_info");
  ...
  deleteConfigurationInfo(emp:any)
  {
   
    let body = new URLSearchParams();
    body.append('id', this.selectedDeactivateEmployee.id);
    body.append('action', 'deleteConfigurationInfo');
    this.http.post(environment.apikey + '/generateJsonUrl.php', body)
      .subscribe((data: any) => {
       this.loadData();
      }, error => {
